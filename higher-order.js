...
//Code Here
let postTaxPrices = prices.map(num => (num*0.07)+num);
...
//Code Here
let totalPopulation = populations.reduce((num,val) => num+val)
...
//Code Here
let myStrongest = monstersInYourPocket.filter(current => current.CP > 200)
...
let ordersTotal = orders.filter(num => (num.price*num.tax)+num.price);
...
let bobsTotal = purchases.filter((curr) => curr.owner === 'Bob').reduce((num,val) => {return num+val.price}, 0);
